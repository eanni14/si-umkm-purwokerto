...
export default function ProfilePage() {
    ...
    useEffect(() => {
        // Jika proses autentikasi selesai dan tidak ada user, redirect
        if (!authLoading && !user) {
            router.push('/login');
            return;
        }
        
        // Jika ada user, ambil data produknya
        if (user) {
    ...
    // Tampilkan pesan loading selama proses autentikasi atau pengambilan data
            ...
            <div className="max-w-4xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
                    ...
                    <div className="mt-6 flex gap-4">
                         <a href="/dashboard/my-products" className="bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
                            Manajemen Produk
                        </a>
                        ...
                        <ul className="space-y-4">
                            {products.map(product => (
                                <li key={product.id} className="border-b pb-4 flex justify-between items-center">
                                    ...
                                    <a href={`/dashboard/edit-product/${product.id}`} className="text-sm text-indigo-600 hover:underline">
                                        Edit
                                    </a>
                                </li>
                            ))}
